...
def load_trans_data(args):
    ...
    # test data
    x_test = np.concatenate([val_real, val_fake])
    test_dataset = val_datas

    # eval data
    x_eval = np.concatenate([test_real, test_fake])
    eval_dataset = test_datas
    print('data setting......')
    return train_real, x_test,y_test_fscore,ratio, test_dataset, x_eval,y_eval_fscore,eval_ratio,eval_dataset
...
def my_train_anomaly_detector(x_train, x_test, y_test, ratio, test_dataset):
    print('start....')
    tc_obj = tc.TransClassifierTabular(args,x_train.shape[1])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--demo_model', default='model_epoch_10.pth', type=str)
...
# python3 train_ad_tabular.py --n_rots=256 --batch_size=4 --n_epoch=10 --d_out=32 --ndf=8 --dataset=thyroid
    ...
    if args.dataset == 'thyroid' or args.dataset == 'arrhythmia':
        ...
        PATH = 'checkpoints/kdd_checkpoint/' + args.demo_model
        print('demo test')
