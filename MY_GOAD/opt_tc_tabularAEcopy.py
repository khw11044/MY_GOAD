...
def save_model(folder_name,ds,epoch,model,optimizer,er,thresh):
...
class TransClassifierTabular():
    def __init__(self, args):
        ...
        self.aeNet = AEmodel.autoencoder(self.d_out).cuda()
    ...
    def demo_classifier(self,x_eval,y_eval,eval_ratio,eval_dataset,PATH):
        ...
        model.eval()
        sum_zs = torch.zeros((self.ndf // 4, self.n_rots)).cuda()
        ...
        with torch.no_grad():
            ...
            print(y_pred, y_true)
            save_scv(dir,self.ds,eval_dataset,np.round(val_probs_rots,3),np.round(thresh,3),y_pred, epoch)
            make_graph(dir,self.ds,epoch,False) # 새로운 make_graph함수 말들면 됨 demo 데이터가 100개니까 다 확인하는걸로 
    ...
    def fit_trans_classifier(self, train_xs, x_test, y_test, ratio, test_data):
        ...
        error_list = []
        model = self.netC
        optimizer1 = self.optimizerC
        optimizer2 = self.optimizerAE
        ...
        for epoch in range(self.n_epoch):
            ...
            with torch.no_grad():
                ...
                for i in range(0, len(x_test), self.batch_size):
                    ...
                    zs, fs = model(xs)
                    zs = zs.permute(0, 2, 1)
                    diffs = ((zs.unsqueeze(2) - means) ** 2).sum(-1) #+ ((xs - fs)**2)   # 추가 

                    diffs_eps = self.eps * torch.ones_like(diffs)
                    diffs = torch.max(diffs, diffs_eps)
                    logp_sz = torch.nn.functional.log_softmax(-diffs, dim=2)                    

                    val_probs_rots[idx] = -torch.diagonal(logp_sz, 0, 1, 2).cpu().data.numpy() + torch.mean(((xs - fs)**2),1).cpu().numpy() # Score = - log P(x)
                ...
                make_graph(dir,self.ds,epoch,False)
            ...
            print('==========Save Model==========')
            # save_model(dir,self.ds,epoch,model,optimizer,er,thresh)
